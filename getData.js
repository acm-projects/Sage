...
const inputDocument = "transcript.4-7-24.pdf";
...
const startDetection = (async function () 
{
    var jobFound = false;
    ...
    while (jobFound == false)
    {
        ...
        for (var message of sqsReceivedResponse.Messages)
        {
            ...
            if (String(rekMessage.JobId).includes(String(response.JobId)))
            {
                ...
                jobFound = true;
                var operationResults = await getResults(rekMessage.JobId);
...
const getResults = async (JobID) =>
{
    ...
    while (finished == false)
    {
        ...
        {
            if (blockArray[i].BlockType == "WORD")
            {
                wordArray.push(blockArray[i].Text);
            }
            // if (blockArray[i].BlockType == "LINE"); -- Variant to push Lines instead of words, would have to change makeJSON()
            // {
            //     wordArray.push(blockArray[i].Text);
            // }
        }
    ...
    const transcriptData = makeJSON(wordArray);

    console.log("Data being uploaded: ");
    const dbPutParams = {
        TableName: "Users",
        Item: {
            UtdID: transcriptData.id,
            Name: transcriptData.name,
            GPA: transcriptData.cumGPA,
...
function makeJSON(words) // Returns TJ
{
        ...
        if (noTransfer && words[i] == "Transfer") // Getting transfer credits
        {
            ...
            while (words[i] != "Test" && words[i] != "History")
            {
                ...
                if (keywords.includes(words[i])) // Season found (semester found)
                {
                     ...
                     * the courses begin */
                    courseName = ""; // Initialize courseName to store/concatenate name of courses
    ...
    // console.log(TJ);
    // for (let k = 0; k < TJ.semesters.length; k++)
    // {
    //     console.log(TJ.semesters[k]);
    // }
    // return TJ;
}